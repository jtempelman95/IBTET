...
for knominal = 1e4
for nvii = [5 1]
datapath =[ '..\Data\F22\11-11-22_data\Fmin1e1_4band_loop_EVI_kVI_Hertzian_knominal_',num2str(log10(knominal)),'_RVI',num2str(RVI),'_Ncyc',num2str(Ncyc),'_Ndof',num2str(Ndof)];
if exist(datapath)
%     disp('Filepath exists! Press a key to continue')
%     pause;
%     disp('Are you sure you want to overwrite:')
...
clf
subplot(1,2,1)
imagesc(abs(latticedata.vc));%colormap(linspecer.^2)
set(gca,'colorscale','log')
title(['(2) FampIDX = ',num2str(fampidx),', Amp = ',num2str(round(Famp))])
plothelper.tickformat
% ------------------------------------------------------
