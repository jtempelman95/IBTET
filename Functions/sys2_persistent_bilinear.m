function dx=sys2_persistent_bilinear(t,x,Minv,As,excitation, VIforce,lattice)
% ---------------------------
% Simulaiton of bilinear VI lattice
% ---------------------------
            ...
            for ii = 1:numel(VIsites)
                if numel(kc)>1, KC = kc(ii);else,KC=kc;end
                   ...
                   UDOTMINUS_pers(ii,Index_pers(SITE)) = ddelta;  
                       ...
                       if sided ==1;
                             fnl_hertz =             -KC*( max(-delta-d(SITE),0) .^(1) )   ; 
                       elseif sided == -1 
                             fnl_hertz =             -KC*( -max(delta-d(SITE),0) .^(1) ) ; 
                       elseif sided == 2
                             fnl_hertz =                 -KC*(max(-delta-d(SITE),0) .^(1) - max(delta-d(SITE),0) .^(1))   ;
                            % fnl_hertz =            fnl_hertz -fnl_hertz*ddelta/delta*1e-3   ; % Arbitrary Damping
                       end
